...
function bindAdaptive(el,s){
  ...
  function show(p){ current=p; pool.forEach(function(x){ x.node.hidden=(x!==p); });
    var ex=p.node.querySelector(".ap-explain"); if(ex) ex.hidden=true;        // mastery may revisit an item
    var sc=p.node.querySelector(".ap-scaffold"); if(sc) sc.hidden=true;
    var o=p.node.querySelector(".opt"); if(o) setTimeout(function(){ o.focus(); },50); }
  function next(){
    if(finished) return;
    if(mastery){ if(pool.every(function(x){ return x.done; })) return finalize(); }   // all items resolved
    ...
    var pick;
    if(mastery && preferSkill){   // on wrong: a DIFFERENT item of the same skill (ALEKS; probe-resistant)
    ...
    if(!pick){
      if(strategy==="bkt"){ avail.sort(function(a,b){ return a.d-b.d; }); pick=avail[0]; }   // mastery: easy-to-hard
      else { pick=G.pickByTargetSuccess(function(p){ return est.pCorrect(p.d); }, avail, {target:target}, rng); } // flow: ZPD
    }
    if(pick) _shownAt[s.id+"."+pick.id]=Date.now();   // S1 — per-item latency start
    show(pick);
  }
  function finalize(){ if(finished) return; finished=true;
    pool.forEach(function(x){ x.node.hidden=true; });
    var ratio=answered?correctN/answered:0, ok;
    if(mastery && s.score_mode==="mastery"){   // all resolved items mastered => 100%, else 0
    ...
    XAPI.emit("finalize",{ok:ok,score:correctN,max:answered});                                    // telemetry (W5b)
    applyActions(ok?s.on_correct:s.on_wrong);
    if(fb){ var msg=ok?(s.feedback&&s.feedback.correct||""):(s.feedback&&s.feedback.incorrect||"");
  ...
  pool.forEach(function(p){
    ...
    check.addEventListener("click",function(){
      ...
      // S1 — adaptive pool reported per-item: each item is a separate cmi.interactions record.
      recordInteraction(s, ok, [sel.dataset.opt], correct, s.id+"."+p.id, "mcq");
      if(strategy==="bkt") est.observe(ok); else est.observe(p.d, ok);   // update proficiency
      XAPI.emit("adaptive.observe", strategy==="bkt"                       // telemetry (W5b): ability OR mastery
      ...
      if(ok){
        ...
        if(mastery){
          if(p.cw >= (s.max_consecutive_wrong||5)){ p.done=true; p.mastered=false; updHud(); setTimeout(next,60); } // ALEKS: 5 consecutive wrong
          else if(s.related_retry){ p.done=false; preferSkill=skillOf[p.id]||null; updHud(); setTimeout(next,60); }   // different question, same skill
          else { opts.forEach(function(o){ o.disabled=false; o.classList.remove("selected","wrong","correct"); });
                 if(check) check.disabled=false; }                                                         // retry the same question
        } else { p.done=true; updHud(); setTimeout(next,60); }   // existing "sample" behavior (legacy)
      }
    });
  });
  updHud(); next();
