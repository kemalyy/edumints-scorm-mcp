...
.opt-wrap{display:flex;flex-direction:column;gap:var(--space-2)}
...
/* .opt-fb (mcq seçime özel gerekçe) scen-conseq ile AYNI görünüm — selektör paylaşılır */
.scen-conseq,.opt-fb{font-size:14px;color:var(--c-muted);padding:var(--space-2) var(--space-4);
...
function checkChoice(el,s){
  ...
  el.querySelectorAll(".opt").forEach(function(o){
    ...
    // Seçime özel gerekçe: sarmalayıcı İÇİNDEKİ .opt-fb'yi aç (scen-conseq deseni).
    // .opt-wrap kontrolü şart — sarmalayıcısız şıkta parentNode .options'tır ve
    // querySelector BAŞKA bir şıkkın gerekçesini bulurdu.
    var w=o.parentNode;
    if(o.classList.contains("selected")&&w&&w.classList&&w.classList.contains("opt-wrap")){
      var ofb=w.querySelector(".opt-fb"); if(ofb) ofb.hidden=false;
    }
  });
...
function bindAdaptive(el,s){
  ...
  var mastery=(s.loop_mode==="mastery");   // W4b-gen: mastery loop (opt-in; default "sample")
  var skillOf={}; for(var k in (cfg.items||{})){ if(cfg.items[k]) skillOf[k]=cfg.items[k].skill; }
  ...
  var pool=[]; root.querySelectorAll(".ap-item").forEach(function(node){
    pool.push({id:node.dataset.item, d:parseFloat(node.dataset.difficulty)||0, node:node, done:false, cw:0, mastered:false}); });
  var cap=(cfg.max_items&&cfg.max_items>0)?Math.min(cfg.max_items,pool.length):pool.length;
  var answered=0, correctN=0, finished=false, current=null, preferSkill=null;
  ...
  function updHud(){ if(prog) prog.textContent=(mastery?correctN:answered)+" / "+(mastery?pool.length:cap); if(lvl) lvl.textContent=levelText(); }
  function show(p){ current=p; pool.forEach(function(x){ x.node.hidden=(x!==p); });
    // Ustalık döngüsü AYNI öğeye geri döner: cevap arayüzü yeniden cevaplanabilir olmalı. Aksi hâlde
    // ilk yanlıştan sonra şıklar/kontrol butonu disabled kalır, öğe hiç done olmaz ve pool.every(done)
    // asla gerçekleşmez → ekran BİTMEZ (related_retry varsayılan açık olduğu için ana yol budur).
    if(mastery && !p.done){
      p.node.querySelectorAll(".opt").forEach(function(o){
        o.disabled=false; o.classList.remove("selected","wrong","correct"); });
      var ck=p.node.querySelector(".ap-check"); if(ck) ck.disabled=false;
    }
    var ex=p.node.querySelector(".ap-explain"); if(ex) ex.hidden=true;        // açıklama = bottom-out, yalnız çözülünce
    // Kazanılmış ipucu tekrarda GÖRÜNÜR kalır — "yanlışta ipucu, sonra tekrar dene" döngüsünün amacı bu.
    var sc=p.node.querySelector(".ap-scaffold");
    if(sc) sc.hidden=!(mastery && s.scaffold_on_wrong && (p.cw||0)>0);
    var o=p.node.querySelector(".opt"); if(o) setTimeout(function(){ o.focus(); },50); }
  function next(){
    if(finished) return;
    if(mastery){ if(pool.every(function(x){ return x.done; })) return finalize(); }   // tüm öğeler çözüldü
    else { if(answered>=cap) return finalize(); }
    ...
    var pick;
    if(mastery && preferSkill){   // yanlışta: aynı beceriden FARKLI öğe (ALEKS; probing direnci)
      var same=avail.filter(function(p){ return skillOf[p.id]===preferSkill && p.id!==(current&&current.id); });
      if(same.length) pick=same[Math.floor(rng()*same.length)];
      preferSkill=null;
    }
    if(!pick){
  ...
  function finalize(){ if(finished) return; finished=true;
    ...
    var ratio=answered?correctN/answered:0, ok;
    if(mastery && s.score_mode==="mastery"){   // tüm öğeler ustalanmışsa %100, aksi halde 0
      var resolved=pool.filter(function(x){ return x.done; }).length;
      ok=(resolved===pool.length && correctN===resolved);
    } else { ok=ratio>=(cfg.pass_ratio||0.6); }
  ...
  pool.forEach(function(p){
    ...
    check.addEventListener("click",function(){
      ...
      // Probing direnci: ustalık döngüsünde YANLIŞ cevap doğruyu sızdırmaz. Öğe çözülene dek
      // (doğru cevap VEYA max_consecutive_wrong ile pes) ne .correct işareti ne de explain_html
      // gösterilir — yerine scaffold (ipucu) gelir. "sample" modunda reveal her zaman true → eski davranış.
      var reveal = !mastery || ok || ((p.cw||0)+1 >= (s.max_consecutive_wrong||5));
      opts.forEach(function(o){ o.disabled=true;
        if(reveal && correct.indexOf(o.dataset.opt)>=0 && s.feedback && s.feedback.show_correct) o.classList.add("correct");
      ...
      var ex=node.querySelector(".ap-explain"); if(ex) ex.hidden=!reveal;   // bottom-out: yalnız çözülünce
      if(ok){
        p.done=true; p.mastered=true;
        updHud(); setTimeout(next,60);
      } else {
        p.cw=(p.cw||0)+1;
        if(mastery && s.scaffold_on_wrong){ var sc=node.querySelector(".ap-scaffold"); if(sc) sc.hidden=false; }
        if(mastery){
          // p.cw öğe-başına birikimlidir; doğru cevap öğeyi anında done yaptığı için birikimli == ARDIŞIK.
          if(p.cw >= (s.max_consecutive_wrong||5)){ p.done=true; p.mastered=false; updHud(); setTimeout(next,60); } // ALEKS: 5 ardışık yanlış
          else if(s.related_retry){ p.done=false; preferSkill=skillOf[p.id]||null; updHud(); setTimeout(next,60); }   // farklı soru, aynı beceri
          else { opts.forEach(function(o){ o.disabled=false; o.classList.remove("selected","wrong","correct"); });
                 if(check) check.disabled=false; }                                                         // aynı soruyu tekrar dene
        } else { p.done=true; updHud(); setTimeout(next,60); }   // mevcut "sample" davranışı (legacy)
      }
    });
  });
  updHud(); next();
