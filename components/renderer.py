...
def _r_mcq(s) -> str:
    def _opt(o) -> str:
        ...
        # Sarmalayıcı YALNIZ gerekçesi olan şıkta — feedback_html kullanmayan kursun
        # çıktısı bayt-aynı kalır (runtime da .opt-wrap kontrolüyle eşleşir).
        if not o.feedback_html:
            return btn
        return (
            f'<div class="opt-wrap">{btn}'
            f'<div class="opt-fb rich" hidden>{sanitize(o.feedback_html)}</div></div>'
        )
